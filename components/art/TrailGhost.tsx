...
const TRAIL_DECAY = 0.91;   // alpha multiplier per frame — lower = shorter trails
const TRAIL_MAX = 50;

export function TrailGhost({ size = 300 }: { size?: number }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      // Slight persistence instead of full clear — ghosting effect
      ...
      pts.forEach(p => {
        ...
        // Draw node head (brighter, on top of trail)
        let nodeR = p.r;
        if (p.isDot && m > 0.15) {
